...
public class Tools {
    ...
    private static Field[] getAllFields(Class<?> cls) {
        ...
        Class<?> tmp = cls;
        // if cls is an interface, getSuperclass might end up returning null.
        while (cls != null && cls != Object.class && cls != Thread.class) {
            ...
            if (obj.getClass().getComponentType() == int.class) {
                ...
                for (int i = 0; i < tmp.length; i++) {
                    ...
                    if (ref == null) {
                        ...
                        int index = tree.getIndex(ref);
                        if (index == -1) {
                            Debug.debug(Debug.MODULE_MERKLE, "Missing reference parent=(%s, %s) element[%d](%s,%s)",
                                    obj.getClass(), obj, i, ref.getClass(), ref);
                            throw new RuntimeException("Missing reference parent=("
                                    + obj.getClass() + "," + obj + ") element[" + i + "]" + " obj: " + ref + " (which is a " + ref.getClass() + ")");
                        }
                        out.writeInt(index);
                    }
                }
        ...
        if (!obj.getClass().isArray()) {
            if (!(obj instanceof Class)) {
                ...
                for (int i = 0; i < field.length; i++) {
                    ...
                    if (!type.isPrimitive() && !type.isEnum() && field[i].getAnnotation(MerkleTreeDirectSerializable.class) == null) {
                ...
                Field[] field = getAllFields((Class<?>) obj);
                for (int i = 0; i < field.length; i++) {
                    field[i].setAccessible(true);
                    int modifiers = field[i].getModifiers();
                    Class<?> type = field[i].getType();
    ...
    public static Object createNewObject(Class<?> cls)
        ...
        return getSerializableConstructor(cls).newInstance((Object[]) null);
    }
