// $Id: ViewChange.java 722 2011-07-04 09:24:35Z aclement $
...
public class ViewChange extends MacArrayMessage {
     ...
     **/
    public ViewChange(Parameters param, long view, long ppview,
                      long ccpseq, long scpseq, long pseq,
                      long ppseq,
        ...
        viewNo = view;
        prepreparedView = ppview;

        if (prepreparedView >= viewNo) {
            throw new RuntimeException("invalid view change, preprepare too" +
                    "big: " + prepreparedView + " " + viewNo);
        }
        ...
        // place the last preprepared seq no
        UnsignedTypes.longToBytes(prepreparedView, bytes, offset);
        offset += UnsignedTypes.uint32Size;
    ...
    public ViewChange(byte[] bits, Parameters param) {
        ...
        // read the prepreapred view number
        prepreparedView = UnsignedTypes.bytesToLong(bits, offset);
        offset += UnsignedTypes.uint32Size;
        ...
        for (int j = 0; j < histories.length; j++) {
            ...
            histories[j] = HistoryDigest.fromBytes(tmp, param);
        }
    ...
    protected long prepreparedView;

    public long getPrepreparedView() {
        return prepreparedView;
    }
    ...
    public long maxCompatibleSequenceNumber(ViewChange vc) {
        ...
        for (int i = 0; i < batchHashes.length && offset < vc.getBatchHashes().length; i++, offset++) {
            if (batchHashes[i].equals(vc.getBatchHashes()[offset]) && histories[i].equals(vc.getHistories()[offset])) {
                result++;
            }
        }
        return result;
    ...
    private static int computeSize(Parameters param, Digest d1, Digest d4,
        ...
        int size = MessageTags.uint32Size + MessageTags.uint32Size +
                ...
                MessageTags.uint32Size + MessageTags.uint32Size +
                Digest.size(param) + Digest.size(param);
    ...
    public static void main(String args[]) {
        ...
        ViewChange vmb = new ViewChange(param, 1, 0, 2, 2, 4, 8,
        ...
        vmb = new ViewChange(param, 134, 12, 22, 22, 24, 28, d, d, ppHash, new HistoryDigest[0], 235);
        //System.out.println("\ninitial: "+vmb.toString());
        UnsignedTypes.printBytes(vmb.getBytes());
        vmb2 = new ViewChange(vmb.getBytes(), param);
        //System.out.println("\nsecondary: "+vmb2.toString());
        UnsignedTypes.printBytes(vmb2.getBytes());
