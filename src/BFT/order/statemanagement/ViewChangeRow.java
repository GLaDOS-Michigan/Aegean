// $Id: ViewChangeRow.java 396 2010-06-02 00:03:43Z aclement $
...
public class ViewChangeRow {
    ...
    public ViewChangeRow(Parameters param) {
        ...
        smallestView = 0;
        lastRemoved = 0;
        viewChanges = new ViewChangeCertificate[parameters.getOrderCount()];
        pendingAcks = new ViewChangeAck[parameters.getOrderCount()];
    }
    ...
    public void observeViewChange(ViewChange vc, boolean amprimary) {
        ...
        if (viewChanges[index] == null ||
            ...
            viewChanges[index] = new ViewChangeCertificate(parameters, vc.getView(), amprimary);
        }
        ...
        for (int i = 0; i < pendingAcks.length; i++) {
            if (pendingAcks[i] != null && pendingAcks[i].getView() == vc.getView()) {
    ...
    public void addViewChangeAck(ViewChangeAck vca) {
        ...
        if (viewChanges[index] != null && viewChanges[index].getView() == vca.getView()) {
            viewChanges[index].addAck(vca);
        } else if (pendingAcks[send] == null || pendingAcks[send].getView() < vca.getView()) {
    ...
    public void addCommitView(CommitView cv, boolean amprimary) {
        int index = index(cv.getView());
        if (cv.getView() < lastReceived) {
            return;
        }
        ...
        viewChanges[index].addCommitView(cv);
    ...
    public ViewChangeCertificate getCertificate(long view) {
        ...
        if (smallestView <= view) {
            ...
            for (int i = 0; i < viewChanges.length; i++) {
                if (viewChanges[i] != null && (viewChanges[i].getView() < tmp || tmp == view)) {
    ...
    protected int index(long sn) {
        return (int) (sn % parameters.getOrderCount());
    }
}
