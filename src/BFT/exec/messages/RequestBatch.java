    ...
    public RequestBatch(byte[] bytes, int count, Parameters param) {
        ...
        while (num < count) {
            ...
            if (tag == MessageTags.SignedRequestCore) {
                tmp = new byte[size + SignedMessage.computeAuthenticationSize() + VerifiedMessageBase.getOffset()];
            } else if (tag == MessageTags.FilteredRequestCore) {
                tmp = new byte[size
                        + MacSignatureMessage.computeAuthenticationSize(parameters.getFilterCount(), parameters.getExecutionCount(), parameters)
