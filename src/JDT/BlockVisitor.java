...
public class BlockVisitor extends ASTVisitor {
    ...
    private String getNextCounterName() {
        ++unique_id;
        return "mantisblock_" + getFullClassName() + "_" + unique_id;
    }

    public void endVisit(Block b) {
        StructuralPropertyDescriptor spd = b.getLocationInParent();
        Block newBlock = (Block) addCounterIncrStmt(b);
        if (newBlock != null) {
            b.getParent().setStructuralProperty(spd, newBlock);
        }
