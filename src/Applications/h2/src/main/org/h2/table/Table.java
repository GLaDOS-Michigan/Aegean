...
public abstract class Table extends SchemaObjectBase {
    ...
    private final MTMapWrapper columnMap = new MTMapWrapper(
            new HashMap<String, Column>(), false, true, false);
    ...
    Table(Schema schema, int id, String name, boolean persistIndexes,
          boolean persistData) {
        MerkleTreeInstance.add(this);
        MerkleTreeInstance.add(columnMap);
    ...
    public ObjectArray<DbObject> getChildren() {
        ObjectArray<DbObject> children = ObjectArray.newInstance(false);
     ...
     * @param session  the session
     * @param rows     a list of row pairs of the form old row, new row, old row, new
     *                 row,...
     */
    ...
    public void checkColumnIsNotReferenced(Column col) throws SQLException {
        for (int i = 0; constraints != null && i < constraints.size(); i++) {
            ...
            if (constraint.containsColumn(col)) {
                throw Message.getSQLException(
                        ErrorCode.COLUMN_MAY_BE_REFERENCED_1, constraint
                                .getSQL());
            }
        }
        ObjectArray<Index> indexes = getIndexes();
    ...
    public Row getTemplateRow(boolean addToTree) {
        Value[] values = new Value[columns.length];
        if (addToTree) {
            MerkleTreeInstance.add(values);
        }
        return new Row(addToTree, values, memoryPerRow);
    }
    ...
    Row getNullRow() {
        synchronized (this) {
            if (nullRow == null) {
                nullRow = new Row(true, new Value[columns.length], 0);
                for (int i = 0; i < columns.length; i++) {
    ...
    public void addConstraint(Constraint constraint) {
        if (constraints == null || constraints.indexOf(constraint) < 0) {
            MerkleTreeInstance.update(this);
            constraints = add(true, constraints, constraint);
        }
    }
    ...
    public void addSequence(Sequence sequence) {
        sequences = add(true, sequences, sequence);
    }
    ...
    public void addTrigger(TriggerObject trigger) {
        triggers = add(true, triggers, trigger);
    }

    private <T> ObjectArray<T> add(boolean addInTree, ObjectArray<T> list, T obj) {
        if (list == null) {
            list = ObjectArray.newInstance(addInTree);
            if (addInTree) {
    ...
    private boolean fireRow(Session session, Row oldRow, Row newRow,
                            boolean beforeAction, boolean rollback) throws SQLException {
        if (triggers != null) {
            for (TriggerObject trigger : triggers) {
                boolean done = trigger.fireRow(session, oldRow, newRow,
                        beforeAction, rollback);
                if (done) {
                    return true;
                }
            }
