...
public class Function extends Expression implements FunctionCall {
    ...
    protected Function(Database database, FunctionInfo info) {
        ...
        if (info.parameterCount == VAR_ARGS) {
            varArgs = ObjectArray.newInstance(false);
        } else {
    ...
    private Value getSimpleValue(Session session, Value v0, Expression[] argList) throws SQLException {
        ...
        switch (info.type) {
            ...
            case CURRENT_DATE:
                // need to normalize
                result = ValueDate.get(new Date(0));//System.currentTimeMillis()));
                break;
            case CURTIME:
            case CURRENT_TIME:
                // need to normalize
                result = ValueTime.get(new Time(0));//System.currentTimeMillis()));
                break;
            case NOW:
            case CURRENT_TIMESTAMP: {
                ValueTimestamp vt = ValueTimestamp.getNoCopy(new Timestamp(0));//System.currentTimeMillis()));
                if (v0 != null) {
