...
public class Comparison extends Condition {
    ...
    private Expression getCast(Expression expr, int targetDataType,
                               long precision, int scale, int displaySize, Session session)
            throws SQLException {
    ...
    public Expression optimize(Session session) throws SQLException {
        ...
        if (right == null) {
            ...
            try {
                ...
                if (left instanceof ExpressionColumn) {
                    if (right.isConstant()) {
                        ...
                        Expression test = getCast(right, left.getType(), left
                                .getPrecision(), left.getScale(), left
                                .getDisplaySize(), session);
            ...
            if (lt == rt) {
                ...
                long precision = Math.max(left.getPrecision(), right
                        .getPrecision());
                int scale = Math.max(left.getScale(), right.getScale());
                int displaySize = Math.max(left.getDisplaySize(), right
                        .getDisplaySize());
                if (dataType != lt) {
    ...
    public void createIndexConditions(Session session, TableFilter filter) {
        ...
        if (addIndex) {
            if (l != null) {
                filter.addIndexCondition(IndexCondition.get(compareType, l,
                        right));
            } else if (r != null) {
    ...
    Expression getAdditional(Session session, Comparison other, boolean and) {
        if (compareType == other.compareType && compareType == EQUAL) {
            ...
            if (and) {
                ...
                if (rc && r2c && l.equals(l2)) {
                    return new ConditionIn(db, left, ObjectArray.newInstance(
                            false, right, other.right));
                } else if (rc && l2c && l.equals(r2)) {
                    return new ConditionIn(db, left, ObjectArray.newInstance(
                            false, right, other.left));
                } else if (lc && r2c && r.equals(l2)) {
                    return new ConditionIn(db, right, ObjectArray.newInstance(
                            false, left, other.right));
                } else if (lc && l2c && r.equals(r2)) {
                    return new ConditionIn(db, right, ObjectArray.newInstance(
                            false, left, other.left));
                }
            }
        }
        return null;
