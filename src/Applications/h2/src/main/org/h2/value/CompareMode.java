...
public class CompareMode {
    ...
    private transient static CompareMode lastUsed;

    private transient String name;
    private transient int strength;
    private transient Collator collator;
    private transient SmallLRUCache<String, CollationKey> collationKeys;
    ...
    public boolean equalsChars(String a, int ai, String b, int bi,
                               boolean ignoreCase) {
        if (collator != null) {
            return compareString(a.substring(ai, ai + 1), b.substring(bi,
                    bi + 1), ignoreCase) == 0;
        }
        char ca = a.charAt(ai);
        char cb = b.charAt(bi);
