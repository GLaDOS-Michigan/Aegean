...
public abstract class Value {
    ...
    // private static SoftReference<Value[]> softCache = new
    // SoftReference<Value[]>(null);
    ...
    static Value cache(Value v) {
        // TODO commented for Merkle Trees
        ...
        switch (type) {
            ...
            case BYTE: {
                switch (getType()) {
                    case BOOLEAN:
                        return ValueByte
                                .get(getBoolean().booleanValue() ? (byte) 1
                                        : (byte) 0);
                    case SHORT:
            ...
            case INT: {
                switch (getType()) {
                    case BOOLEAN:
                        return ValueInt
                                .get(getBoolean().booleanValue() ? 1 : 0);
                    case BYTE:
            ...
            case LONG: {
                switch (getType()) {
                    case BOOLEAN:
                        return ValueLong.get(getBoolean().booleanValue() ? 1
                                : 0);
                    case BYTE:
            ...
            case DOUBLE: {
                switch (getType()) {
                    case BOOLEAN:
                        return ValueDouble.get(getBoolean().booleanValue() ? 1
                                : 0);
                    case BYTE:
            ...
            case FLOAT: {
                switch (getType()) {
                    case BOOLEAN:
                        return ValueFloat.get(getBoolean().booleanValue() ? 1
                                : 0);
                    case BYTE:
            ...
            case DATE: {
                switch (getType()) {
                    case TIME:
                        return ValueDate
                                .get(new Date(getTimeNoCopy().getTime()));
                    case TIMESTAMP:
                        return ValueDate.get(new Date(getTimestampNoCopy()
                                .getTime()));
                }
                break;
            }
            case TIME: {
                switch (getType()) {
                    case DATE:
                        ...
                        return ValueTime
                                .get(new Time(getDateNoCopy().getTime()));
                    case TIMESTAMP:
                        // need to normalize the year, month and day
                        return ValueTime.get(new Time(getTimestampNoCopy()
                                .getTime()));
                }
                break;
            }
            case TIMESTAMP: {
                switch (getType()) {
                    case TIME:
                        return ValueTimestamp.getNoCopy(new Timestamp(
                                getTimeNoCopy().getTime()));
                    case DATE:
                        return ValueTimestamp.getNoCopy(new Timestamp(
                                getDateNoCopy().getTime()));
                }
                break;
            }
            case BYTES: {
            ...
            case BLOB: {
                switch (getType()) {
                    case BYTES:
                        return ValueLob.createSmallLob(Value.BLOB,
                                getBytesNoCopy());
                }
                break;
            }
            ...
            switch (type) {
                ...
                case BOOLEAN: {
                    if (s.equalsIgnoreCase("true") || s.equalsIgnoreCase("t")
                        ...
                        // convert to a number, and if it is not 0 then it is
                        // true
                        return ValueBoolean
                                .get(new BigDecimal(s).signum() != 0);
                    }
                }
                case BYTE:
                ...
                case TIMESTAMP:
                    return ValueTimestamp.getNoCopy(ValueTimestamp
                            .parseTimestamp(s.trim()));
                case BYTES:
                    return ValueBytes.getNoCopy(ByteUtils
                ...
                case CLOB:
                    return ValueLob.createSmallLob(CLOB, StringUtils
                            .utf8Encode(s));
                case BLOB:
                    return ValueLob.createSmallLob(BLOB, ByteUtils
                            .convertStringToBytes(s.trim()));
                case ARRAY:
     ...
     * @throws SQLException
     */
    protected SQLException throwUnsupportedExceptionForType()
            throws SQLException {
        throw Message
                .getUnsupportedException(DataType.getDataType(getType()).name);
    }
