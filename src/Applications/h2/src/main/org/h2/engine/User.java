...
public class User extends RightOwner {
     ...
     *
     * @param password    true if the password (actually the salt and hash) should be
     *                    returned
    ...
    public String getCreateSQL(boolean password, boolean ifNotExists) {
        ...
        if (password) {
            buff.append(" SALT '").append(ByteUtils.convertBytesToString(salt))
                    .append("' HASH '").append(
                    ByteUtils.convertBytesToString(passwordHash))
                    .append('\'');
        } else {
