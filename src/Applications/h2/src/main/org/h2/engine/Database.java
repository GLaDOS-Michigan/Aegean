...
public class Database implements DataHandler {
    ...
    private final MTMapWrapper roles = new MTMapWrapper(
            new HashMap<String, Role>(), false, true, false);
    private final MTMapWrapper users = new MTMapWrapper(
            new HashMap<String, User>(), false, true, false);
    private final MTMapWrapper settings = new MTMapWrapper(
            new HashMap<String, Setting>(), false, true, false);
    private final MTMapWrapper schemas = new MTMapWrapper(
            new HashMap<String, Schema>(), false, true, false);
    private final MTMapWrapper rights = new MTMapWrapper(
            new HashMap<String, Right>(), false, true, false);
    private final MTMapWrapper functionAliases = new MTMapWrapper(
            new HashMap<String, FunctionAlias>(), false, true, false);
    private final MTMapWrapper userDataTypes = new MTMapWrapper(
            new HashMap<String, UserDataType>(), false, true, false);
    private final MTMapWrapper aggregates = new MTMapWrapper(
            new HashMap<String, UserAggregate>(), false, true, false);
    private final MTMapWrapper comments = new MTMapWrapper(
            new HashMap<String, Comment>(), false, true, false);

    // private final MTCollectionWrapper userSessions = new
    // MTCollectionWrapper(Collections
    // .synchronizedSet(new HashSet<Session>()), true, false);

    private final MTCollectionWrapper userSessions = new MTCollectionWrapper(
            new HashSet<Session>(), true, false);
    ...
    private transient LogSystem log;
    private transient WriterThread writer;
    private transient HashMap<Integer, Storage> storageMap = New.hashMap();
    private boolean starting;
    private transient DiskFile fileData, fileIndex;
    private transient TraceSystem traceSystem;
    private transient DataPage dummy;
    private int fileLockMethod;
    private Role publicRole;
    private transient long modificationDataId;
    ...
    private transient DatabaseEventListener eventListener;
    ...
    private transient DatabaseCloser delayedCloser;
    private boolean recovery;
    private volatile boolean closing;
    private boolean ignoreCase;
    private boolean deleteFilesOnDisconnect;
    private transient String lobCompressionAlgorithm;
    private boolean optimizeReuseResults = true;
    private transient String cacheType;
    private boolean indexSummaryValid = true;
    private transient String accessModeLog, accessModeData;
    private boolean referentialIntegrity = true;
    private boolean multiVersion;
    private transient DatabaseCloser closeOnExit;
    private transient Mode mode = Mode.getInstance(Mode.REGULAR);
    ...
    private transient SmallLRUCache<String, String[]> lobFileListCache = SmallLRUCache
            .newInstance(128);
    private boolean autoServerMode;
    private transient Server server;
    private MTMapWrapper linkConnections;
    private transient TempFileDeleter tempFileDeleter = TempFileDeleter
            .getInstance();
    private transient PageStore pageStore;
    private boolean usePageStoreSet, usePageStore;
    private transient Properties reconnectLastLock;
    private transient volatile long reconnectCheckNext;
    ...
    private transient final Object reconnectSync = new Object();
    private int cacheSize;
    private boolean compactFully;
    private transient SourceCompiler compiler;
    ...
    public Database(String name, ConnectionInfo ci, String cipher)
        ...
        MerkleTreeInstance.add(roles);
        MerkleTreeInstance.add(users);
        MerkleTreeInstance.add(settings);
        MerkleTreeInstance.add(schemas);
        MerkleTreeInstance.add(rights);
        MerkleTreeInstance.add(functionAliases);
        MerkleTreeInstance.add(userDataTypes);
        MerkleTreeInstance.add(aggregates);
        MerkleTreeInstance.add(comments);
        MerkleTreeInstance.add(objectIds);
        MerkleTreeInstance.add(objectIds.data);
        MerkleTreeInstance.add(userSessions);
        MerkleTreeInstance.add(lobSyncObject);
        ...
        MerkleTreeInstance.add(this.databaseShortName);
        this.cipher = cipher;
        ...
        this.usePageStore = ci.getProperty("PAGE_STORE", SysProperties
                .getPageStore());
        ...
        this.cacheType = StringUtils.toUpperEnglish(ci.removeProperty(
                "CACHE_TYPE", SysProperties.CACHE_TYPE_DEFAULT));
    ...
    private void openDatabase(int traceLevelFile, int traceLevelSystemOut,
                              boolean closeAtVmShutdown) throws SQLException {
        try {
     ...
     * @return true if the call was successful, false if another connection was
     * faster
     */
        ...
        try {
            if (pending == reconnectChangePending) {
                ...
                if (now > reconnectCheckNext) {
                    ...
                    reconnectCheckNext = now
                            + SysProperties.RECONNECT_CHECK_DELAY;
                }
                return true;
            }
            ...
            reconnectCheckNext = System.currentTimeMillis() + 2
                    * SysProperties.RECONNECT_CHECK_DELAY;
            old = lock.save();
            if (pending) {
    ...
    public static boolean exists(String name) {
        return FileUtils.exists(name + Constants.SUFFIX_PAGE_FILE)
                | FileUtils.exists(name + Constants.SUFFIX_DATA_FILE);
    }
    ...
    private synchronized void open(int traceLevelFile, int traceLevelSystemOut)
        ...
        if (persistent) {
            ...
            if (autoServerMode) {
                if (readOnly || fileLockMethod == FileLock.LOCK_NO
                        || fileLockMethod == FileLock.LOCK_SERIALIZED) {
                    throw Message
                            .getUnsupportedException("autoServerMode && (readOnly || fileLockMethod == NO"
            ...
            if (!readOnly && fileLockMethod != FileLock.LOCK_NO) {
                lock = new FileLock(traceSystem, lockFileName,
            ...
            while (isReconnectNeeded() && !beforeWriting()) {
                ...
                // until we can write (file are not open - no need to
                // re-connect)
            }
            if (exists) {
        ...
        Column columnId = new Column(s, Value.INT);
        MerkleTreeInstance.update(columnId);
        columnId.setNullable(false);
        cols.add(columnId);
        String s2 = "HEAD";
        MerkleTreeInstance.add(s2);
        cols.add(new Column(s2, Value.INT));
        String s3 = "TYPE";
        MerkleTreeInstance.add(s3);
        cols.add(new Column(s3, Value.INT));
        String s4 = "SQL";
        MerkleTreeInstance.add(s4);
        cols.add(new Column(s4, Value.STRING));
        int headPos = 0;
        if (pageStore != null) {
            headPos = pageStore.getSystemTableHeadPos();
        }
        data.tableName = "SYS";
        MerkleTreeInstance.add(data.tableName);
        data.id = 0;
        ...
        Column[] t = new Column[]{columnId};
        MerkleTreeInstance.add(t);
        IndexColumn[] pkCols = IndexColumn.wrap(t);
        String indexName = "SYS_ID";
        MerkleTreeInstance.add(indexName);
        metaIdIndex = meta.addIndex(systemSession, indexName, 0, pkCols,
        ...
        ObjectArray<MetaRecord> records = ObjectArray.newInstance(false);
        while (cursor.next()) {
    ...
    public void createScanIndices() {
        ObjectArray<Table> tables = mainSchema.getAllTablesAndViews();
        for (int i = 0; i < tables.size(); i++) {
            TableData t = (TableData) tables.get(i);
            t.rebuildScanIndex();
        }
    }

    private void startServer(String key) throws SQLException {
        server = Server.createTcpServer("-tcpPort", "0", "-tcpAllowOthers",
    ...
    private void addDefaultSetting(Session session, int type,
        ...
        // TODO: the string should not be added (rather the SetTypes class
    ...
    public Storage getStorage(int id, DiskFile file) {
        ...
        if (storage != null) {
            if (SysProperties.CHECK && storage.getDiskFile() != file) {
                Message.throwInternalError(storage.getDiskFile() + " != "
                        + file + " id:" + id);
            }
        } else {
    ...
    private synchronized void addMeta(Session session, DbObject obj)
        ...
        if (id > 0 && !starting && !obj.isTemporary()) {
            Row r = meta.getTemplateRow(true);
    ...
    private synchronized void closeOpenFilesAndUnlock(boolean flush)
        ...
        if (lock != null) {
            if (fileLockMethod == FileLock.LOCK_SERIALIZED) {
                ...
                try {
                    Thread
                            .sleep((int) (SysProperties.RECONNECT_CHECK_DELAY * 1.1));
    ...
    public ObjectArray<UserAggregate> getAllAggregates() {
        return ObjectArray.newInstance(false, aggregates.values());
    }

    public ObjectArray<Comment> getAllComments() {
        return ObjectArray.newInstance(false, comments.values());
    }

    public ObjectArray<FunctionAlias> getAllFunctionAliases() {
        return ObjectArray.newInstance(false, functionAliases.values());
    }
    ...
    public ObjectArray<Right> getAllRights() {
        return ObjectArray.newInstance(false, rights.values());
    }

    public ObjectArray<Role> getAllRoles() {
        return ObjectArray.newInstance(false, roles.values());
    }
    ...
    public ObjectArray<Table> getAllTablesAndViews(boolean includeMeta) {
        ...
        ObjectArray<Table> list = ObjectArray.newInstance(false);
        for (Object o : schemas.values()) {
            Schema schema = (Schema) o;
    ...
    public ObjectArray<Schema> getAllSchemas() {
        initMetaTables();
        return ObjectArray.newInstance(false, schemas.values());
    }

    public ObjectArray<Setting> getAllSettings() {
        return ObjectArray.newInstance(false, settings.values());
    }

    public ObjectArray<Storage> getAllStorages() {
        return ObjectArray.newInstance(false, storageMap.values());
    }

    public ObjectArray<UserDataType> getAllUserDataTypes() {
        return ObjectArray.newInstance(false, userDataTypes.values());
    }

    public ObjectArray<User> getAllUsers() {
        return ObjectArray.newInstance(false, users.values());
    }
    ...
    public synchronized void removeSchemaObject(Session session,
        ...
        if (!starting) {
            ...
            if (invalid != null) {
                obj.getSchema().add(obj);
                throw Message.getSQLException(ErrorCode.CANNOT_DROP_2, obj
                        .getSQL(), invalid);
            }
            obj.removeChildrenAndResources(session);
        }
    ...
    public synchronized String getTempTableName(Session session) {
        String tempName;
        do {
            tempName = Constants.TEMP_TABLE_PREFIX + session.getId() + "_"
                    + nextTempTableId++;
        }
    ...
    public void deleteLogFileLater(String fileName) throws SQLException {
        if (fileLockMethod == FileLock.LOCK_SERIALIZED) {
            // need to truncate the file, because another process could keep it
            // open
        ...
        switch (lockMode) {
            ...
            case 0:
                if (multiThreaded) {
                    ...
                    // MULTI_THREADED is not supported
                    throw Message.getSQLException(
    ...
    public TableLinkConnection getLinkConnection(String driver, String url,
        ...
        if (linkConnections == null) {
            linkConnections = new MTMapWrapper(
                    new HashMap<TableLinkConnection, TableLinkConnection>(),
                    false, true, false);
        }
        ...
        try {
            ...
            while (true) {
                ...
                if (System.currentTimeMillis() > now
                        + SysProperties.RECONNECT_CHECK_DELAY * 10) {
     ...
     *
     * @param maxLastChange indexes that were changed afterwards are not flushed; 0 to
     *                      flush all indexes
     */
