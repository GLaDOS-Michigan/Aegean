...
class ResultDiskBuffer implements ResultExternal {
    ...
    static class ResultDiskTape {
         ...
         */
        ObjectArray<Value[]> buffer = ObjectArray.newInstance(false);
    }
    ...
    ResultDiskBuffer(Session session, SortOrder sort, int columnCount) throws SQLException {
        ...
        if (sort != null) {
            tapes = ObjectArray.newInstance(false);
        } else {
    ...
    public void reset() {
        if (sort != null) {
            for (ResultDiskTape tape : tapes) {
                tape.pos = tape.start;
                tape.buffer = ObjectArray.newInstance(false);
            }
        } else {
            mainTape.pos = FileStore.HEADER_LENGTH;
            mainTape.buffer = ObjectArray.newInstance(false);
        }
    }
