...
public class Parser {
    ...
    private Prepared parse(String sql, boolean withExpectedList)
        ...
        initialize(sql);
        if (withExpectedList) {
            expectedList = ObjectArray.newInstance(false);
        } else {
            expectedList = null;
        }
        parameters = ObjectArray.newInstance(false);
        currentSelect = null;
    ...
    private TransactionCommand parseCommit() throws SQLException {
        ...
        if (readIf("TRANSACTION")) {
            command = new TransactionCommand(session,
                    TransactionCommand.COMMIT_TRANSACTION);
    ...
    private TransactionCommand parseRollback() throws SQLException {
        ...
        if (readIf("TRANSACTION")) {
            command = new TransactionCommand(session,
                    TransactionCommand.ROLLBACK_TRANSACTION);
            command.setTransactionName(readUniqueIdentifier());
            return command;
        }
        if (readIf("TO")) {
    ...
    private Prepared parsePrepare() throws SQLException {
        if (readIf("COMMIT")) {
            TransactionCommand command = new TransactionCommand(session,
                    TransactionCommand.PREPARE_COMMIT);
            command.setTransactionName(readUniqueIdentifier());
            return command;
        }
        String procedureName = readAliasIdentifier();
        if (readIf("(")) {
            ObjectArray<Column> list = ObjectArray.newInstance(false);
            for (int i = 0; ; i++) {
    ...
    private Update parseUpdate() throws SQLException {
        ...
        read("SET");
        if (readIf("(")) {
            ObjectArray<Column> columns = ObjectArray.newInstance(false);
            do {
    ...
    private IndexColumn[] parseIndexColumnList() throws SQLException {
        ObjectArray<IndexColumn> columns = ObjectArray.newInstance(false);
        do {
            IndexColumn column = new IndexColumn();
            column.columnName = readColumnIdentifier();
            // MT: ADDED FOR TPC-W
            MerkleTreeInstance.add(column.columnName);
    ...
    private String[] parseColumnList() throws SQLException {
        ObjectArray<String> columns = ObjectArray.newInstance(false);
        do {
            String columnName = readColumnIdentifier();
    ...
    private Prepared parseHelp() throws SQLException {
        ...
        int i = 0;
        ObjectArray<Value> paramValues = ObjectArray.newInstance(false);
    ...
    private Prepared parseShow() throws SQLException {
        ...
        if (readIf("CLIENT_ENCODING")) {
            ...
            // for PostgreSQL compatibility
            buff
                    .append("'read committed' AS DEFAULT_TRANSACTION_ISOLATION FROM DUAL");
            ...
            buff
                    .append("TABLE_NAME, TABLE_SCHEMA FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA=? ORDER BY TABLE_NAME");
    ...
    private Prepared prepare(Session s, String sql,
                             ObjectArray<Value> paramValues) throws SQLException {
    ...
    private Merge parseMerge() throws SQLException {
        ...
        if (readIf("VALUES")) {
            do {
                ObjectArray<Expression> values = ObjectArray.newInstance(false);
                read("(");
                if (!readIf(")")) {
                    do {
                        if (readIf("DEFAULT")) {
    ...
    private TableFilter readTableFilter(boolean fromOuter) throws SQLException {
        ...
        if (readIf("(")) {
            if (isSelect()) {
                ...
                ObjectArray<Parameter> params = ObjectArray.newInstance(false);
    ...
    private void parseEndOfQuery(Query command) throws SQLException {
        if (readIf("ORDER")) {
            ...
            ObjectArray<SelectOrderBy> orderList = ObjectArray
                    .newInstance(false);
            do {
    ...
    private void parseSelectSimpleSelectPart(Select command)
        ...
        ObjectArray<Expression> expressions = ObjectArray.newInstance(false);
        do {
            if (readIf("*")) {
    ...
    private Select parseSelectSimple() throws SQLException {
        ...
        if (readIf("GROUP")) {
            ...
            ObjectArray<Expression> list = ObjectArray.newInstance(false);
            do {
                Expression expr = readExpression();
                list.add(expr);
            } while (readIf(","));
        ...
        while (true) {
            ...
            if (readIf("LIKE")) {
                ...
                if (SysProperties.OPTIMIZE_IN
                        && !SysProperties.OPTIMIZE_IN_LIST) {
                    recompileAlways = true;
                }
                read("(");
                if (readIf(")")) {
                    ...
                    if (isSelect()) {
                        ...
                        ObjectArray<Expression> v = ObjectArray
                                .newInstance(false);
                        Expression last;
                        do {
                            last = readExpression();
                ...
                if (readIf("ALL")) {
                    ...
                    if (readIf("(") && readIf("+") && readIf(")")) {
                        ...
                        if (r instanceof ExpressionColumn
                            ...
                            ObjectArray<TableFilter> filters = currentSelect
                                    .getTopFilters();
    ...
    private ObjectArray<SelectOrderBy> parseSimpleOrderList()
            throws SQLException {
        ObjectArray<SelectOrderBy> orderList = ObjectArray.newInstance(false);
        do {
            SelectOrderBy order = new SelectOrderBy();
            Expression expr = readExpression();
            order.expression = expr;
    ...
    private JavaFunction readJavaFunction(String name) throws SQLException {
        ...
        Expression[] args;
        ObjectArray<Expression> argList = ObjectArray.newInstance(false);
        int numArgs = 0;
        while (!readIf(")")) {
            if (numArgs++ > 0) {
                read(",");
    ...
    private JavaAggregate readJavaAggregate(UserAggregate aggregate)
            throws SQLException {
        ObjectArray<Expression> params = ObjectArray.newInstance(false);
        do {
    ...
    private Expression readTerm() throws SQLException {
        ...
        switch (currentTokenType) {
            ...
            case PARAMETER:
                ...
                boolean indexed = Character
                        .isDigit(sqlCommandChars[parseIndex]);
                read();
                Parameter p;
                if (indexed && currentTokenType == VALUE
                    ...
                    if (indexedParameterList == null) {
                        ...
                        indexedParameterList = ObjectArray.newInstance(false);
                    }
            ...
            case IDENTIFIER:
                ...
                if (currentTokenQuoted) {
                    ...
                    read();
                    if (equalsToken("X", name) && currentTokenType == VALUE
                            && currentValue.getType() == Value.STRING) {
                        read();
                        byte[] buffer = ByteUtils
                                .convertStringToBytes(currentValue.getString());
                        r = ValueExpression.get(ValueBytes.getNoCopy(buffer));
                    } else if (readIf(".")) {
                        r = readTermObjectDot(name);
                    } else if (equalsToken("CASE", name)) {
                        // CASE must be processed before (,
                        // otherwise CASE(3) would be a function call, which it
                        // is
                        // not
                        if (isToken("WHEN")) {
                            r = readWhen(null);
                        ...
                        read();
                        r = ValueExpression.get(ValueTimestamp
                                .getNoCopy(ValueTimestamp
            ...
            case MINUS:
                ...
                if (currentTokenType == VALUE) {
                    ...
                    // convert Integer.MIN_VALUE to int (-Integer.MIN_VALUE
                    // needed
                    // to be a long)
                    if (r.getType() == Value.LONG
                            && r.getValue(session).getLong() == Integer.MIN_VALUE) {
                        r = ValueExpression
                                .get(ValueInt.get(Integer.MIN_VALUE));
                    }
                    read();
                } else {
            ...
            case OPEN:
                ...
                r = readExpression();
                if (readIf(",")) {
                    ObjectArray<Expression> list = ObjectArray
                            .newInstance(false);
                    list.add(r);
                    do {
                        r = readExpression();
    ...
    private void read() throws SQLException {
        ...
        switch (type) {
            case CHAR_NAME:
                ...
                currentToken = StringCache.getNew(sqlCommand
                        .substring(start, i));
    ...
    private void readDecimal(int start, int i) throws SQLException {
        ...
        try {
            ...
            throw Message.getSQLException(ErrorCode.DATA_CONVERSION_ERROR_1, e,
                    sub);
        }
        checkLiterals(false);
        currentValue = ValueDecimal.get(bd);
    ...
    private Column parseColumn(String columnName) throws SQLException {
        ...
        if (templateColumn != null) {
            ...
            column.setDefaultExpression(session, templateColumn
        ...
        if (comment != null) {
            MerkleTreeInstance.add(comment);
        }
        if (original != null) {
            MerkleTreeInstance.add(original);
        }
        column.setComment(comment);
    ...
    private TransactionCommand parseCheckpoint() throws SQLException {
        ...
        if (readIf("SYNC")) {
            command = new TransactionCommand(session,
                    TransactionCommand.CHECKPOINT_SYNC);
        } else {
            command = new TransactionCommand(session,
    ...
    private Prepared parseAlterTable() throws SQLException {
        ...
        if (readIf("ADD")) {
            ...
            AlterTableSet command = new AlterTableSet(session, table
                    .getSchema(), type);
            ...
            if (readIf("RENAME")) {
                ...
                AlterSequence command = new AlterSequence(session, table
                        .getSchema());
                command.setColumn(column);
    ...
    private CreateTable parseCreateTable(boolean temp, boolean globalTemp,
                                         boolean persistIndexes) throws SQLException {
        boolean ifNotExists = readIfNoExists();
        String tableName = readIdentifierWithSchema();
        MerkleTreeInstance.add(tableName);
            ...
            if (!readIf(")")) {
                do {
                    ...
                    if (c != null) {
                        ...
                        String columnName = readColumnIdentifier();
                        MerkleTreeInstance.add(columnName);
                        Column column = parseColumnForTable(columnName);
