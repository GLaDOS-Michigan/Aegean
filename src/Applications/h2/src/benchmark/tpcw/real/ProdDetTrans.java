...
public class ProdDetTrans extends Transition {

    @Override
    public void execute(User eb, Connection con, Statement stat)
            throws SQLException {
        if (Benchmark.TRACE) {
            System.out.println("Executing ProdDetTrans");
        }
        Vector v = null;
        if (eb.books != null) {
            v = eb.books;
        } else if (!eb.related_item_ids.isEmpty()) {
            v = eb.related_item_ids;
        }

        if (v != null) {
            // Retrieve one of the IID from the books retrieved before.
            // Ask for details about it.
            int i = eb.nextInt(v.size());
            Integer I_ID = ((Integer) v.elementAt(i)).intValue();

            eb.mybook = TPCW_Database.getBook(I_ID, con, eb.dbID);
        }
    }
