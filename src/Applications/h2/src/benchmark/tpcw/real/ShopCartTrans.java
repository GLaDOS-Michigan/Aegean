...
public class ShopCartTrans extends Transition {
    ...
    public void execute(User eb, Connection con, Statement stat)
            throws SQLException {
        if (Benchmark.TRACE) {
            System.out.println("Executing ShopCartTrans");
        }

        if (eb.SHOPPING_ID == -1) {
            eb.SHOPPING_ID = TPCW_Database.createEmptyCart(con, eb.dbID);
        }
        ...
        // TODO: VQ this will need to be modified
        ...
        eb.cart = TPCW_Database.doCart(eb.SHOPPING_ID, null, ids, quantities,
                con, eb.rand, eb.dbID);
