...
public class TPCW_Database {
    ...
    public static String[] getName(int c_id, Connection con, int id)
            throws SQLException {
        ...
        PreparedStatement get_name = con
                .prepareStatement("SELECT c_fname,c_lname FROM customer" + id
                        + " WHERE c_id = ?");
    ...
    public static Book getBook(int i_id, Connection con, int id)
            throws SQLException {
        Book book = null;
        // Prepare SQL
    ...
    public static Customer getCustomer(String UNAME, Connection con, int id)
            throws SQLException {
        Customer cust = null;
        // Prepare SQL
        PreparedStatement statement = con
                .prepareStatement("SELECT * FROM customer" + id + ", address"
                        + id + ", country" + id + " WHERE customer" + id
                        + ".c_addr_id = address" + id + ".addr_id AND address"
                        + id + ".addr_co_id = country" + id
                        + ".co_id AND customer" + id + ".c_uname = ?");
    ...
    public static Vector<Book> doSubjectSearch(String search_key,
                                               Connection con, int id) throws SQLException {
        ...
        PreparedStatement statement = con
                ...
                .prepareStatement("SELECT * FROM item" + id + ", author" + id
                        + " WHERE item" + id + ".i_a_id = author" + id
                        + ".a_id AND item" + id
                        + ".i_subject = ? ORDER BY item" + id + ".i_title "
                        + limit(50));
    ...
    public static Vector<Book> doTitleSearch(String search_key, Connection con,
        ...
        // Prepare SQL
        PreparedStatement statement = con.prepareStatement("SELECT * FROM item"
                + id + ", author" + id + " WHERE item" + id
                + ".i_a_id = author" + id + ".a_id AND item" + id
                + ".i_title LIKE ? ORDER BY item" + id + ".i_title "
                + limit(50));
    ...
    private static String limit(int nb) {
        if (TPCW_Benchmark.USE_H2) {
            return "LIMIT " + nb;
        } else if (TPCW_Benchmark.USE_DERBY) {
            return "FETCH FIRST " + nb + " ROWS ONLY";
        }
    ...
    public static Vector<Book> doAuthorSearch(String search_key,
        ...
        // Prepare SQL
        PreparedStatement statement = con
                .prepareStatement("SELECT * FROM author" + id + ", item" + id
                        + " WHERE author" + id + ".a_lname LIKE ? AND item"
                        + id + ".i_a_id = author" + id + ".a_id ORDER BY item"
                        + id + ".i_title " + limit(50));
    ...
    public static Vector<ShortBook> getNewProducts(String subject,
        ...
        PreparedStatement statement = con
                .prepareStatement("SELECT i_id, i_title, a_fname, a_lname "
                        + "FROM item" + id + ", author" + id + " WHERE item"
                        + id + ".i_a_id = author" + id + ".a_id " + "AND item"
                        + id + ".i_subject = ? " + "ORDER BY item" + id
                        + ".i_pub_date DESC,item" + id + ".i_title "
                        + limit(50));
    ...
    public static Vector<ShortBook> getBestSellers(String subject,
        ...
        PreparedStatement statement = con
                .prepareStatement("SELECT i_id, i_title, a_fname, a_lname "
                        ...
                        + "AND order_line"
                        + id
                        + ".ol_o_id > (SELECT MAX(o_id)-3333 FROM orders"
                        + id
                        + ") "
                        + "AND item"
                        + id
    ...
    public static void getRelated(int i_id, Vector<Integer> i_id_vec,
                                  Vector<String> i_thumbnail_vec, Connection con, int id)
            throws SQLException {
        // Prepare SQL
        PreparedStatement statement = con
                .prepareStatement("SELECT J.i_id,J.i_thumbnail from item"
                        + id
                        + " I, item"
                        + id
                        + " J where (I.i_related1 = J.i_id or I.i_related2 = J.i_id or I.i_related3 = J.i_id or I.i_related4 = J.i_id or I.i_related5 = J.i_id) and I.i_id = ?");
    ...
    public static void adminUpdate(int i_id, double cost, String image,
                                   String thumbnail, Connection con, int id) throws SQLException {
        // Prepare SQL
        PreparedStatement statement = con
                .prepareStatement("UPDATE item"
                        + id
                        + " SET i_cost = ?, i_image = ?, i_thumbnail = ?, i_pub_date = CURRENT DATE WHERE i_id = ?");

        // Set parameter
        // MerkleTreeInstance.add(cost);
        // MerkleTreeInstance.add(image);
        // MerkleTreeInstance.add(thumbnail);
        ...
        PreparedStatement related = con.prepareStatement("SELECT ol_i_id "
                ...
                + " AND orders"
                + id
                + ".o_id > (SELECT MAX(o_id)-10000 FROM orders"
                + id
                + ")"
                + " AND order_line"
                + id
...

        {
            ...
            statement = con
                    .prepareStatement("UPDATE item"
                            + id
                            + " SET i_related1 = ?, i_related2 = ?, i_related3 = ?, i_related4 = ?, i_related5 = ? WHERE i_id = ?");

            // Set parameter
            // MerkleTreeInstance.add(related_items[0]);
            // MerkleTreeInstance.add(related_items[1]);
            // MerkleTreeInstance.add(related_items[2]);
            // MerkleTreeInstance.add(related_items[3]);
            // MerkleTreeInstance.add(related_items[4]);
            // MerkleTreeInstance.add(i_id);
    ...
    public static String GetUserName(int C_ID, Connection con, int id)
            throws SQLException {
        String u_name = null;
        // Prepare SQL
        PreparedStatement get_user_name = con
                .prepareStatement("SELECT c_uname FROM customer" + id
                        + " WHERE c_id = ?");
    ...
    public static String GetPassword(String C_UNAME, Connection con, int id)
            throws SQLException {
        String passwd = null;
        // Prepare SQL
        PreparedStatement get_passwd = con
                .prepareStatement("SELECT c_passwd FROM customer" + id
                        + " WHERE c_uname = ?");
    ...
    private static int getRelated1(int I_ID, Connection con, int id)
            throws SQLException {
        int related1 = -1;
        PreparedStatement statement = con
                .prepareStatement("SELECT i_related1 FROM item" + id
                        + " where i_id = ?");
        statement.setInt(1, I_ID);
        ResultSet rs = statement.executeQuery();
        rs.next();
    ...
    public static Order GetMostRecentOrder(String c_uname,
                                           Vector<OrderLine> order_lines, Connection con, int id)
            throws SQLException {
...

        {
            ...
            PreparedStatement get_most_recent_order_id = con
                    .prepareStatement("SELECT o_id " + "FROM customer" + id
                            + ", orders" + id + " WHERE customer" + id
                            + ".c_id = orders" + id + ".o_c_id "
                            + "AND c_uname = ? " + "ORDER BY o_date, orders"
                            + id + ".o_id DESC " + limit(1));
...

        {
            ...
            PreparedStatement get_order_lines = con
                    .prepareStatement("SELECT * " + "FROM order_line" + id
                            + ", item" + id + " WHERE ol_o_id = ? "
                            + "AND ol_i_id = i_id");
    ...
    public static int createEmptyCart(Connection con, int id)
            throws SQLException {
        int SHOPPING_ID = 0;
        // while(success == false) {
    ...
    public static Cart doCart(int SHOPPING_ID, Integer I_ID,
                              ...
                              Random rand, int id) throws SQLException {
        Cart cart = null;
        if (I_ID != null) {
            addItem(con, SHOPPING_ID, I_ID.intValue(), id);
        }
        refreshCart(con, SHOPPING_ID, ids, quantities, id);
        addRandomItemToCartIfNecessary(con, SHOPPING_ID, rand, id);
        resetCartTime(con, SHOPPING_ID, id);
        cart = TPCW_Database.getCart(con, SHOPPING_ID, 0.0, id);
    ...
    public static void addItem(Connection con, int SHOPPING_ID, int I_ID, int id)
            throws SQLException {
        // Prepare SQL
        ...
        if (rs.next()) {
            ...
            // MerkleTreeInstance.add(currqty);
            // MerkleTreeInstance.add(SHOPPING_ID);
            // MerkleTreeInstance.add(I_ID);
    ...
    private static void refreshCart(Connection con, int SHOPPING_ID,
                                    Vector<Integer> ids, Vector<Integer> quantities, int id)
            throws SQLException {
        int i;
    ...
    private static void addRandomItemToCartIfNecessary(Connection con,
                                                       int SHOPPING_ID, Random r, int id) throws SQLException {
        ...
        if (rs.getInt(1) == 0) {
            ...
            related_item = getRelated1(rand_id, con, id);
            addItem(con, SHOPPING_ID, related_item, id);
        }
    ...
    private static void resetCartTime(Connection con, int SHOPPING_ID, int id)
            throws SQLException {
        PreparedStatement statement = con
    ...
    public static Cart getCart(int SHOPPING_ID, double c_discount,
                               Connection con, int id) throws SQLException {
        Cart mycart = null;
        mycart = getCart(con, SHOPPING_ID, c_discount, id);
        con.commit();
        return mycart;
    }
    ...
    private static Cart getCart(Connection con, int SHOPPING_ID,
                                double c_discount, int id) throws SQLException {
        Cart mycart = null;
        PreparedStatement get_cart = con.prepareStatement("SELECT * "
                + "FROM shopping_cart_line" + id + ", item" + id
                + " WHERE scl_i_id = item" + id + ".i_id AND scl_sc_id = ?");
        get_cart.setInt(1, SHOPPING_ID);
        ResultSet rs = get_cart.executeQuery();
    ...
    // doesn't exist, but ...
    public static void refreshSession(int C_ID, Connection con, int id)
            throws SQLException {
        // Prepare SQL
        PreparedStatement updateLogin = con
                .prepareStatement("UPDATE customer"
                        + id
                        + " SET c_login = CURRENT TIMESTAMP, c_expiration = CURRENT TIMESTAMP + 2 HOURS WHERE c_id = ?");
    ...
    public static Customer createNewCustomer(Customer cust, Connection con,
        ...
        java.sql.Date date = new java.sql.Date(cust.c_since.getTime());
        java.sql.Date date2 = new java.sql.Date(cust.c_last_visit.getTime());
        java.sql.Date date3 = new java.sql.Date(cust.c_login.getTime());
        java.sql.Date date4 = new java.sql.Date(cust.c_expiration.getTime());
        java.sql.Date date5 = new java.sql.Date(cust.c_birthdate.getTime());
        // MerkleTreeInstance.add(cust.c_fname);
        // MerkleTreeInstance.add(cust.c_lname);
        // MerkleTreeInstance.add(cust.c_phone);
        // MerkleTreeInstance.add(cust.c_email);
        // MerkleTreeInstance.add(date);
        // MerkleTreeInstance.add(date2);
        // MerkleTreeInstance.add(date3);
        // MerkleTreeInstance.add(date4);
        // MerkleTreeInstance.add(cust.c_discount);
        // MerkleTreeInstance.add(cust.c_balance);
        // MerkleTreeInstance.add(cust.c_ytd_pmt);
        // MerkleTreeInstance.add(date5);
        // MerkleTreeInstance.add(cust.c_data);
        ...
        insert_customer_row.setDate(9, date);
        insert_customer_row.setDate(10, date2);
        insert_customer_row.setDate(11, date3);
        insert_customer_row.setDate(12, date4);
        ...
        insert_customer_row.setDate(16, date5);
        ...
        PreparedStatement get_max_id = con
                .prepareStatement("SELECT max(c_id) FROM customer" + id);
    ...
    public static BuyConfirmResult doBuyConfirm(int shopping_id,
                                                ...
                                                Date cc_expiry, String shipping, Connection con, Random rand, int id)
            throws SQLException {

        BuyConfirmResult result = new BuyConfirmResult();
        double c_discount = getCDiscount(con, customer_id, id);
        result.cart = getCart(con, shopping_id, c_discount, id);
        int ship_addr_id = getCAddr(con, customer_id, id);
        result.order_id = enterOrder(con, customer_id, result.cart,
                ship_addr_id, shipping, c_discount, rand, id);
        enterCCXact(con, result.order_id, cc_type, cc_number, cc_name,
                cc_expiry, result.cart.SC_TOTAL, ship_addr_id, id);
        clearCart(con, shopping_id, id);
        con.commit();
        return result;
    }
    ...
    public static BuyConfirmResult doBuyConfirm(int shopping_id,
                                                ...
                                                Connection con, Random rand, int id) throws SQLException {
    ...
    // DB query time: .05s
    private static double getCDiscount(Connection con, int c_id, int id)
            throws SQLException {
        double c_discount = 0.0;
        // Prepare SQL
        PreparedStatement statement = con
                .prepareStatement("SELECT c_discount FROM customer" + id
    ...
    // DB time: .05s
    private static int getCAddrID(Connection con, int c_id, int id)
            throws SQLException {
        int c_addr_id = 0;
        // Prepare SQL
        PreparedStatement statement = con
                .prepareStatement("SELECT c_addr_id FROM customer" + id
                        + " WHERE customer" + id + ".c_id = ?");
    ...
    private static int getCAddr(Connection con, int c_id, int id)
            throws SQLException {
        int c_addr_id = 0;
        // Prepare SQL
        PreparedStatement statement = con
    ...
    private static void enterCCXact(Connection con,
                                    ...
                                    int ship_addr_id, int id) throws SQLException {
        ...
        PreparedStatement statement = con
                .prepareStatement("INSERT into cc_xacts"
                        ...
                        + "VALUES (?, ?, ?, ?, ?, ?, CURRENT DATE, (SELECT co_id FROM address"
                        + id + ", country" + id
                        + " WHERE addr_id = ? AND addr_co_id = co_id))");

        // Set parameter
        // MerkleTreeInstance.add(cc_type);
        // MerkleTreeInstance.add(cc_name);
        // MerkleTreeInstance.add(cc_expiry);
        // MerkleTreeInstance.add(total);
    ...
    public static void clearCart(Connection con, int shopping_id, int id)
            throws SQLException {
    ...
    private static int enterAddress(
            ...
            String zip, String country, int id) throws SQLException {
        ...
        PreparedStatement get_co_id = con
                .prepareStatement("SELECT co_id FROM country" + id
                        + " WHERE co_name = ?");
        ...
        PreparedStatement match_address = con
                .prepareStatement("SELECT addr_id FROM address" + id
        ...
        if (!rs.next()) {// We didn't match an address in the addr table
            ...
            PreparedStatement get_max_addr_id = con
                    .prepareStatement("SELECT max(addr_id) FROM address" + id);
    ...
    private static int enterOrder(Connection con, int customer_id, Cart cart,
        ...
        PreparedStatement insert_row = con
                .prepareStatement("INSERT into orders" + id
        ...
        // MerkleTreeInstance.add(cart.SC_SUB_TOTAL);
        // MerkleTreeInstance.add(cart.SC_TOTAL);
        // MerkleTreeInstance.add(shipping);
        // MerkleTreeInstance.add(O_DATE);
        ...
        insert_row.setInt(8, getCAddrID(con, customer_id, id));
        insert_row.setInt(9, ship_addr_id);

        PreparedStatement get_max_id = con
                .prepareStatement("SELECT count(o_id) FROM orders" + id);
        ...
        while (e.hasMoreElements()) {
            ...
            int stock = getStock(con, cart_line.scl_i_id, id);
            if ((stock - cart_line.scl_qty) < 10) {
                setStock(con, cart_line.scl_i_id, stock - cart_line.scl_qty
                        + 21, id);
            } else {
                setStock(con, cart_line.scl_i_id, stock - cart_line.scl_qty, id);
            }
        }
        return o_id;
    }
    ...
    private static void addOrderLine(Connection con, int ol_id, int ol_o_id,
        ...
        // MerkleTreeInstance.add(ol_comment);
    ...
    public static int getStock(Connection con, int i_id, int id)
            throws SQLException {
        int stock = 0;
        PreparedStatement get_stock = con
                .prepareStatement("SELECT i_stock FROM item" + id
                        + " WHERE i_id = ?");
    ...
    private static void setStock(Connection con, int i_id, int new_stock, int id)
            throws SQLException {
        PreparedStatement update_row = con.prepareStatement("UPDATE item" + id
    ...
    public static void verifyDBConsistency(Connection con, int id)
            throws SQLException {
        int this_id;
        int id_expected = 1;
        // First verify customer table
        PreparedStatement get_ids = con
                .prepareStatement("SELECT c_id FROM customer" + id);
        ...
        get_ids = con.prepareStatement("SELECT i_id FROM item" + id);
        ...
        get_ids = con.prepareStatement("SELECT addr_id FROM address" + id);
        rs = get_ids.executeQuery();
        while (rs.next()) {
