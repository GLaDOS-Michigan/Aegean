...
public class H2Statements implements DBStatements {
    @Override
    ...
    public String createEmptyCart2() {
        return "INSERT into shopping_cart (sc_id, sc_time) "
                + "VALUES (?, CURRENT_TIMESTAMP)";
    }

    public String doAuthorSearch() {
        return "SELECT * FROM author, item WHERE substring(soundex(author.a_lname),0,4)=substring(soundex(?),0,4) AND item.i_a_id = author.a_id ORDER BY item.i_title LIMIT 50";
    }

    public String doSubjectSearch() {
        return "SELECT * FROM item, author WHERE item.i_a_id = author.a_id AND item.i_subject = ? ORDER BY item.i_title LIMIT 50";
    }

    public String doTitleSearch() {
        return "SELECT * FROM item, author WHERE item.i_a_id = author.a_id AND substring(soundex(item.i_title),0,4)=substring(soundex(?),0,4) ORDER BY item.i_title LIMIT 50";
    }
    ...
    public String enterCCXact() {
        return "INSERT into cc_xacts (cx_o_id, cx_type, cx_num, cx_name, cx_expire, cx_xact_amt, cx_xact_date, cx_co_id) "
    ...
    public String refreshSession() {
        return "UPDATE customer SET c_login = CURRENT_TIMESTAMP, c_expiration = DATEADD('HOUR', 2, CURRENT_TIMESTAMP) WHERE c_id = ?";
    }
    ...
    public int convertStatementToCode(String statement) {
        ...
        } else if (statement.equals(commitTransaction())) {
            return code_commit();
        }
