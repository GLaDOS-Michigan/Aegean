...
package Applications.tpcw_webserver.rbe.util;
...
public class StrStrPattern extends StringPattern {
    ...
    public int find(String s, int start, int end) {
        ...
        if (i == -1) {
//    System.err.println("find() returns -1, because pattern not found, tag.p = " + this.p + "   String s = " + s + "\n\n\n");
            return (-1);
        }
        // FIXME:  This is slower than needed, when the
        //  string is much longer than end.
        else if (i >= (end + p.length())) {
//    System.err.println("find() returns -1, because i>=(end+p.length()), tag.p = " + this.p);
            return (-1);
        } else {
            this.start = i;
