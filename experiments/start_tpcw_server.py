...
for i in range(0, num_verifiers):
    ...
    cmd_template = 'java %s -Djava.library.path=. -Xmx1024m -cp lib/log4j-1.2.15.jar:lib/bft.jar:lib/FlexiCoreProvider-1.6p3.signed.jar:lib/CoDec-build17-jdk13.jar:lib/netty-3.2.1.Final.jar BFT.verifier.VerifierBaseNode %%s %s > jh_log/server_verifier_%%s_out 2> jh_log/server_verifier_%%s_err &' % (agent_path, property_file)
