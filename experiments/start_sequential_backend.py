...
for i in range(0, num_servers):
    ...
    cmd_template = 'java -ea %s -Xms2G -Xmx4G -Djava.library.path=. -cp lib/zookeeper-3.2.2.jar:lib/log4j-1.2.15.jar:lib/bft.jar:lib/FlexiCoreProvider-1.6p3.signed.jar:lib/CoDec-build17-jdk13.jar:lib/netty-3.2.1.Final.jar:lib/h2.jar:lib/zookeeper-3.2.2.jar:lib/commons-javaflow-2.0-SNAPSHOT.jar:lib/commons-logging-1.1.3.jar Applications.benchmark.BenchServerUpRight %s %s 1000 10 %s %s 2> jh_log/backend-exec_%s_err >> jh_log/backend-exec_%s_out &' % (#str(debugPort),
...
for i in range(0, num_orders):
    ...
    debugPort = 5000 + i
    #-Xdebug -Xrunjdwp:transport=dt_socket,server=y,suspend=n,address=%s
    cmd_template = 'java %s -Xmx1024m -Djava.library.path=. -cp lib/log4j-1.2.15.jar:lib/bft.jar:lib/FlexiCoreProvider-1.6p3.signed.jar:lib/CoDec-build17-jdk13.jar:lib/netty-3.2.1.Final.jar BFT.order.OrderBaseNode %s %s > jh_log/backend-order_%s_out 2> jh_log/backend-order_%s_err &' % (#str(debugPort),
