...
public class Stack implements Serializable {
	...
	protected float[] fstack;
	protected double[] dstack;
	protected long[] lstack;
	protected Object[] ostack;
	protected Object[] rstack;
	protected int iTop, fTop, dTop, lTop, oTop, rTop;
	...
	public Object popReference() {
		...
		if (log.isDebugEnabled()) {
			...
			log.debug("pop reference " + clazz + "/" + clazzLoader + " [" + o
					+ "] " + getStats());
		}
	...
	public void pushObject(Object o) {		
		if (log.isDebugEnabled()) {
			final String clazz = ReflectionUtils.getClassName(o);
			final String clazzLoader = ReflectionUtils.getClassLoaderName(o);
			log.debug("push object " + clazz + "/" + clazzLoader + " [" + o
					+ "] " + getStats());
		}
	...
	public void pushReference(Object o) {
		...
		if (log.isDebugEnabled()) {
			...
			log.debug("push reference " + clazz + "/" + clazzLoader + " [" + o
					+ "] " + getStats());
		}
	...
	public boolean isEmpty() {
		return iTop == 0 && lTop == 0 && dTop == 0 && fTop == 0 && oTop == 0
				&& rTop == 0;
	}

	public Object[] getAllReferences() {
		// find all objects and references in the stack and return them.
		// TODO what about primitive types?
		Object[] references = new Object[oTop + rTop];

		System.arraycopy(ostack, 0, references, 0, oTop);
		System.arraycopy(rstack, 0, references, oTop, rTop);
	...
	private String getContent() {
		...
		for (int i = 0; i < oTop; i++) {
			...
			sb.append(ReflectionUtils.getClassName(ostack[i])).append('/')
					.append(ReflectionUtils.getClassLoaderName(ostack[i]));
			sb.append('\n');
		}
		sb.append("r[").append(rTop).append("]\n");
		for (int i = 0; i < rTop; i++) {
			sb.append(' ').append(i).append(": ");
			sb.append(ReflectionUtils.getClassName(rstack[i])).append('/')
					.append(ReflectionUtils.getClassLoaderName(rstack[i]));
			sb.append('\n');
		}
	...
	private String getContent1() {
		...
		for (int i = 0; i < oTop; i++) {
			sb.append(' ').append(i).append(": ");
			sb.append(ReflectionUtils.getClassName1(ostack[i])).append('/')
					.append(ReflectionUtils.getClassLoaderName1(ostack[i]));
			sb.append('\n');
		}
		sb.append("r[").append(rTop).append("]\n");
		for (int i = 0; i < rTop; i++) {
			sb.append(' ').append(i).append(": ");
			sb.append(ReflectionUtils.getClassName1(rstack[i])).append('/')
					.append(ReflectionUtils.getClassLoaderName1(rstack[i]));
			sb.append('\n');
		}
